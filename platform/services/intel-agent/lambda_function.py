...
FEEDS = [
    ...
    # India: real government releases + startup/funding/logistics coverage.
    # Synthesis prompt is instructed to skip if nothing in these is relevant —
    # this is a sourced news feed, not a generator of invented opportunities.
    ("PIB India (Govt releases)", "https://pib.gov.in/RssMain.aspx?ModId=6&Lang=1&Regid=3"),
...
<h2>🇮🇳 India Last-Mile & Policy Watch</h2> — ONLY from items sourced from PIB, Inc42, YourStory, or Economic Times in the source material below, and ONLY if relevant to: Indian logistics/last-mile companies (Delhivery, Ecom Express, XpressBees, Shadowfax, Porter, Shiprocket, Ekart, ElasticRun, etc.), ONDC, rural/agri logistics, address/geocoding, or government logistics/MSME/digital-infrastructure policy (PM Gati Shakti, National Logistics Policy, DIGIPIN, PMGSY, etc.). One line each: what happened, why it matters to BiGeo specifically, who's affected. If nothing in today's India-sourced items is relevant, OMIT this entire section — do not pad it, do not invent a story to fill it.
...
If there is truly nothing material today, say so briefly in the Executive Summary and skip the rest. Never fabricate news, funding amounts, market-size figures, scores, or events not present in the source material above. Do not invent "opportunities" or "village-level" statistics that aren't backed by an actual source item below — if asked for that kind of analysis, it belongs in a real research conversation, not a daily news digest.
"""
